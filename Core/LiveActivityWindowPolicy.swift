...
/// 之後就不會再變。把接下來幾句連同各自的起訖時刻一起送出去：
/// - 畫面任何一次重畫（staleDate、解鎖、CarPlay 重新顯示）都能算出那一刻正在唱哪一句，不只推進一句；
...
struct LiveActivityWindowPolicy: Equatable, Sendable {
    /// 視窗涵蓋從現在起幾秒內會開始的句子
    var seconds: TimeInterval = 45
    /// 至少 / 最多帶幾句（時間窗內不夠時仍帶 `minLines` 句；太密時最多 `maxLines` 句，內容有 4 KB 上限）
    var minLines = 2
    var maxLines = 4
    ...
    func upcoming(lines: [LyricLine], currentIndex: Int?, effectivePosition: TimeInterval, now: Date,
        ...
        for j in from..<lines.count {
            ...
            result.append(ActivityUpcomingLine(text: line.text, startAt: start, endAt: end.flatMap { $0 > start ? $0 : nil }))
        }
        return result
    }
}
